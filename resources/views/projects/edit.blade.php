...
<div class="container">
	...
	<form action="{{ route('projects.edit', ['slug' => $project->slug]) }}" method="POST" autocomplete="off">
		...
		<div class="form-group{{ $errors->has('slug') ? ' has-error' : '' }}">
			...
			<input type="text" name="slug" id="slug" class="form-control" placeholder="URL" value="{{ Request::old('slug') ?: $project->slug }}" disabled>
			@if($errors->has('slug'))
		...
		<div class="form-goup{{ $errors->has('body') ? ' has-error' : '' }}">
			...
			<textarea name="body" id="body" rows="5" class="form-control" placeholder="Sadržaj" style="resize:vertical">{{ Request::old('body') ?: $project->body }}</textarea>
			@if($errors->has('body'))
